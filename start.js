...
import { printPresentIAStartupBanner } from "./scripts/presentia-terminal-banner.mjs";
...
// Always resolve the architecture-specific Python module binary path.
// On ARM64 (Ampere, AWS Graviton, Apple Silicon) Node reports process.arch === 'arm64'.
// The presentation-export release ships separate binaries for x64 and arm64.
{
  const arch = process.arch; // e.g. 'x64', 'arm64'
  const archSuffix = arch === "arm64" ? "arm64" : "x64";
  process.env.BUILT_PYTHON_MODULE_PATH = `/app/presentation-export/py/convert-linux-${archSuffix}`;
}
