...
safeLog("[PresentIA] Electron paths initialized:", electronAppPaths);
...
safeLog("[PresentIA] Startup memory:", {
...
const createWindow = () => {
  const mainWindow = new BrowserWindow({
    ...
    icon: path.join(baseDir, "resources/ui/assets/images/presentia_short_filled.png"),
    webPreferences: {
        webSecurity: false,
        ...
        preload: (() => {
          ...
          try {
            if (!fs.existsSync(p)) {
              safeWarn(`[PresentIA] Preload not found at ${p}`);
            }
          } catch (e) {
            safeWarn('[PresentIA] Failed to stat preload path', e);
          }
          return p;
        })(),
    },
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    if (!isSupportedExternalUrl(url)) {
      safeWarn("[PresentIA] Blocked unsupported window open URL.");
      return { action: "deny" };
    }
    ...
    void openExternalUrl(url)
      .then(async (result) => {
        ...
        safeWarn(`[PresentIA] Failed to open external URL: ${result.message || "Unknown error"}`);
        await showOpenTargetErrorDialog({
          parent: mainWindow,
          title: "Could Not Open Link",
          message: "PresentIA could not open this link in your browser.",
          detail: `${result.message || "No application is registered to open this link."}\n\n${url}`,
        });
      })
      .catch((error) => {
        safeWarn("[PresentIA] Failed to handle external URL open:", error);
      });
...
async function startServers(fastApiPort: number, nextjsPort: number) {
  try {
    ...
    if (exportChromiumPath) {
      safeLog("[PresentIA] Export Chromium runtime resolved:", exportChromiumPath);
    } else {
      safeWarn(
        "[PresentIA] Export Chromium runtime was not found; Template Studio slide previews will fail until Chromium is installed."
      );
    }
    if (imageMagickRuntime) {
      safeLog("[PresentIA] ImageMagick runtime resolved:", {
        source: imageMagickRuntime.source,
        binaryPath: imageMagickRuntime.binaryPath,
        homeDir: imageMagickRuntime.homeDir,
      });
    } else {
      safeWarn("[PresentIA] ImageMagick runtime was not found; LiteParse image conversion will fail until it is bundled or installed.");
    }
...
app.whenReady().then(async () => {
    ...
    void initialWindow
      ...
      .catch((error) => {
        if (!initialWindow.isDestroyed()) {
          safeWarn("[PresentIA] Failed to load startup page", error);
        }
      });
  }
  ...
  try {
    ...
    safeWarn("[PresentIA] Failed to persist startup user config", error);
  }
  ...
  try {
    ...
    safeWarn("[PresentIA] Failed to load application URL", error);
    return;
  }
  ...
  if (updateWindow && !updateWindow.webContents.isDestroyed()) {
    safeStderrWrite("[PresentIA] Starting update checker...\n");
