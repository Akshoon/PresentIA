...
const APP_ID = "com.presentia.presentia"
const TEAM_ID = "S6W5C54KL6"
const macTarget = process.env.PRESENTIA_MAC_TARGET
...
const masDevIdentity =
  process.env.PRESENTIA_MAS_DEV_IDENTITY || process.env.CSC_NAME || ""
const masDistributionIdentity =
  process.env.PRESENTIA_MAS_DISTRIBUTION_IDENTITY ||
  process.env.PRESENTIA_MAS_IDENTITY ||
...
function getAppStoreBundleShortVersion() {
  const configuredVersion = process.env.PRESENTIA_APP_STORE_VERSION
  if (configuredVersion) {
    validateAppStoreVersion(configuredVersion, "PRESENTIA_APP_STORE_VERSION")
  ...
  if (!match) {
    throw new Error(
      `Cannot derive an App Store version from package version "${packageMetadata.version}". Set PRESENTIA_APP_STORE_VERSION to three period-separated integers, for example 1.0.0.`
    )
  }
...
function getAppStoreBundleVersion(bundleShortVersion) {
  const configuredBuild = process.env.PRESENTIA_APP_STORE_BUILD
  if (configuredBuild) {
    validateAppStoreBuild(configuredBuild, "PRESENTIA_APP_STORE_BUILD")
...
const config = {
  ...
  asar: false,
  copyright: "Copyright © 2026 PresentIA",
  directories: {
    output: "dist",
  ...
  afterPack,
  mac: {
    artifactName: "PresentIA-${version}.${ext}",
    target: [macTarget || "dmg"],
    category: "public.app-category.productivity",
    hardenedRuntime: false,
    gatekeeperAssess: false,
    identity: macTarget === "mas-dev" ? null : undefined,
    icon: "resources/ui/assets/images/presentia_short_filled.png",
  ...
  nsis: {
    ...
    shortcutName: "PresentIA",
    uninstallDisplayName: "PresentIA",
  },
  appx: {
    identityName: "PresentIAAI.PresentIA",
    publisher: "CN=8A2C57B5-F1C6-473A-93EE-2E9B72134341",
    displayName: "PresentIA",
    publisherDisplayName: "PresentIA Inc.",
    applicationId: "PresentIAAI.PresentIA",
