...
PresentIA can use **Amazon Bedrock** as the text LLM provider for presentation generation, editing, and chat. Configure Bedrock in the app UI (**Settings → Text provider**) or with environment variables / `userConfig.json` (Docker and desktop).
...
In the AWS console: **Bedrock → Inference profiles** (or cross-region inference profiles), copy the full ARN, and paste it into PresentIA’s **Model** field unchanged.

PresentIA passes that string directly to Converse as `modelId`, so both formats work as long as AWS accepts them for your account and region.
...
4. Paste the ARN into PresentIA’s **Model** field (not the short model ID).
...
```bash
docker run -it --name presentia -p 5000:80 \
  -e LLM="bedrock" \
  ...
  -v "./app_data:/app_data" \
  ghcr.io/presentia/presentia:latest
```
...
- **Checks:**
  ...
  - Cross-region inference profiles still use a home region in the ARN; configure PresentIA’s region to match AWS guidance for that profile.
