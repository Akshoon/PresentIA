...
PresentIA’s Electron app is configured for Mac App Store (MAS) distribution with App Sandbox. MAS builds must be produced on macOS because Apple’s signing tools are required.
...
- **Bundle ID:** `com.presentia.presentia`
...
Create a macOS App ID that matches `com.presentia.presentia` (or update `APP_ID` in `electron/build.js` if you use a different ID).
...
|----------|----------|-------------|
| `PRESENTIA_MAC_TARGET` | Set by npm scripts | `mas-dev` or `mas` (do not set manually when using the scripts below) |
| `PRESENTIA_MAS_DEV_IDENTITY` | `mas-dev` | Signing identity, e.g. `Apple Development: Your Name (TEAMID)` |
| `PRESENTIA_MAS_DISTRIBUTION_IDENTITY` | `mas` | Distribution identity, e.g. `Apple Distribution: Your Org (TEAMID)` |
| `PRESENTIA_MAS_IDENTITY` | `mas` | Alias for distribution identity |
| `CSC_NAME` | Either target | Fallback if the target-specific identity vars are unset |
| `PRESENTIA_APP_STORE_VERSION` | `mas` | App Store **version** (`x.y.z`). Required when `package.json` version contains a suffix like `-beta` |
| `PRESENTIA_APP_STORE_BUILD` | `mas` | App Store **build** number. Defaults to the short version if unset |

Example for a beta package version:

```bash
export PRESENTIA_APP_STORE_VERSION=1.0.0
export PRESENTIA_APP_STORE_BUILD=42
```
...
PRESENTIA_MAS_DEV_IDENTITY="Apple Development: Your Name (TEAMID)" \
  npm run build:all:mas-dev

# MAS distribution — for App Store Connect upload
PRESENTIA_MAS_DISTRIBUTION_IDENTITY="Apple Distribution: Your Org (TEAMID)" \
  PRESENTIA_APP_STORE_VERSION=1.0.0 \
...
electron/dist/
  ...
  PresentIA-<version>.pkg # MAS installer for upload
  PresentIA-<version>.dmg # local DMG (default target)
```
...
The default macOS icon is `electron/resources/ui/assets/images/presentia_short_filled.png`.
...
Set `PRESENTIA_APP_STORE_VERSION` to three integers, e.g. `1.0.0`.
