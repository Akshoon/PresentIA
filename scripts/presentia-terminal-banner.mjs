/**
 * Docker / startup terminal banner for PresentIA.
...
function loadAsciiBanner() {
  ...
  const asciiPath = path.join(thisDir, "presentia-ascii.txt");
...
export function printPresentIAStartupBanner(opts = {}) {
  ...
  const title = [
    ...
    "   " +
      ...
      BOLD +
      fgRgb(224, 218, 255, "https://github.com/presentia/presentia"),
    muted("   ─────────────────────────────────────────────────────────"),
    "",
  ].join("\n");
  ...
  const summary = [
    ...
    "",
    "   " + muted("Made with ❤️  by the PresentIA team"),
  ].join("\n");
