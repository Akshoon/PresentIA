/**
 * Download presentia-export release (Linux x64) into repo-root `presentation-export/`.
...
function requestJson(url, redirects = 5) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(
      ...
      {
        headers: {
          "User-Agent": "presentia-presentation-export-sync",
...
function getConverterCandidates(baseDir = targetPyDir) {
  return [
    ...
    path.join(baseDir, "convert-linux-aarch64"),
