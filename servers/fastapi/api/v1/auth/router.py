...
async def setup_credentials(body: AuthCredentialsRequest, request: Request):
    if is_auth_configured():
        raise HTTPException(status_code=409, detail="Las credenciales ya están configuradas")
...
async def login(body: AuthCredentialsRequest, request: Request):
    if not is_auth_configured():
        raise HTTPException(status_code=428, detail="Se requiere configuración inicial de inicio de sesión")

    username = body.username.strip()

    # 1. Verificar si es el admin principal
    if verify_credentials(body.username, body.password):
        token = create_session_token(username)
        response = JSONResponse(
            {"configured": True, "authenticated": True, "username": username, "role": ROLE_ADMIN}
        )
        set_session_cookie(response, token, request)
        return response

    # 2. Verificar si es un usuario extra
    extra_user = verify_extra_user_credentials(body.username, body.password)
    if extra_user:
        token = create_multi_user_session_token(extra_user["username"], extra_user["role"])
        response = JSONResponse(
            {
                "configured": True,
                "authenticated": True,
