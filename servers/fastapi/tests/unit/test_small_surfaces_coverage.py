    ...
    async def runner():
        ...
        with patch.dict(
            ...
            await export_presentation(
                ...
                title="safe",
                export_as="pdf",
                cookie_header="presentia_session=abc; theme=dark",
            )

        pdf_call = mock_pdf.await_args.kwargs
        assert "pdf-maker" in pdf_call["url"]
        assert (
            "#exportCookie=presentia_session%3Dabc%3B+theme%3Ddark"
            in pdf_call["url"]
        )
        assert pdf_call["fastapi_url"] == "https://fast.example"
        assert pdf_call["cookie_header"] == "presentia_session=abc; theme=dark"
