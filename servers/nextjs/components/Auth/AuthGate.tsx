...
export default function AuthGate() {
  ...
  useEffect(() => {
    ...
    if (params.get("reason") === "unauthorized") {
      if (status.configured && !status.authenticated) {
        notify.error("No autorizado", "Inicia sesión para ver esta página.", {
  ...
  const refreshStatus = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("No se pudo cargar el estado de sesión");
      }
      ...
      notify.error(
        ...
        "No pudimos conectar con el servicio de autenticación. Por favor recarga la página."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (isSetupMode && password !== confirmPassword) {
      notify.warning(
        ...
        "Asegúrate de que ambas contraseñas sean iguales antes de continuar."
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          notify.error(
            isSetupMode ? "No se pudo crear la cuenta" : "Error al iniciar sesión",
      ...
      if (isSetupMode) {
        ...
        setPassword("");
        setConfirmPassword("");
        notify.success("Cuenta creada", "Inicia sesión con tu nuevo usuario y contraseña para continuar.", {
          duration: 6000,
        });
        return;
      }
      ...
      notify.success(
        "Sesión iniciada",
        "Bienvenido de nuevo. Cargando tu espacio de trabajo."
      );
    } catch (submitError) {
      console.error(submitError);
      notify.error(
        "Servicio no disponible",
        "El servicio de autenticación no está disponible en este momento. Inténtalo en un momento."
      );
    } finally {
      ...
      <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-white p-6">
        <div className="relative z-10 w-full max-w-md">
          <div className="rounded-2xl border border-[#EDEEEF] bg-white p-8 text-center shadow-xl">
            ...
            <p className="mt-3 font-syne text-sm text-[#000000CC]">Preparando tu espacio de trabajo…</p>
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-white p-6">
      <section className="relative z-10 w-full max-w-xl rounded-2xl border border-[#E1E1E5] bg-white p-7 shadow-xl sm:p-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="mt-1 font-syne text-2xl font-semibold leading-tight text-black sm:text-[26px]">
                {isSetupMode ? "Crea tu cuenta de administrador" : "Inicia sesión para continuar"}
              </h1>
            </div>
          </div>
        </div>
        ...
        <p className="font-syne text-base text-[#000000CC] sm:text-lg">
          {isSetupMode
            ? "Configuración inicial de esta instancia. Usarás el mismo usuario y contraseña en futuras visitas."
            : "Esta instancia está protegida. Ingresa tus credenciales para acceder."}
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          {!isSetupMode && status.configured ? (
            <p className="font-syne text-sm text-[#494A4D]">
              La configuración inicial ya está completa. Usa el usuario y contraseña que configuraste.
            </p>
          ) : null}
