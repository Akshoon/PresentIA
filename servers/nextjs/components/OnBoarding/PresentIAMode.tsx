...
const PresentIAMode = ({ currentStep, setStep }: { currentStep: number, setStep: (step: number) => void }) => {
    ...
    const fetchAvailableModels = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setAvailableModels([]);
                setModelsChecked(true);
                notify.error("No se pudieron cargar los modelos", `El servidor no pudo listar los modelos de ${LLM_PROVIDERS[llmConfig.LLM!]?.label}. Verifica tu API key o endpoint e inténtalo de nuevo.`);
            }
        } catch (error) {
            console.error('Error fetching models:', error);
            notify.error("No se pudieron cargar los modelos", "El servidor no pudo listar los modelos. Verifica tu API key o endpoint e inténtalo de nuevo.");
            setAvailableModels([]);
            setModelsChecked(true);
        } finally {
    ...
    const handleSaveConfig = async () => {
        try {
            if (llmConfig.LLM === 'codex') {
                ...
                if (!isAuthenticated) {
                    notify.error("Inicio de sesión requerido", "Por favor inicia sesión en ChatGPT para continuar.");
                    return;
                }
            }
            const validationError = getLLMConfigValidationError(llmConfig);
            if (validationError) {
                notify.warning("No se puede guardar aún", validationError);
                return;
            }
            ...
            notify.success("Configuración guardada", "Tu configuración se guardó correctamente.");
            setStep(3)
            // router.push("/upload");
        } catch (error) {
            notify.error("No se pudo guardar la configuración", error instanceof Error ? error.message : "Error al guardar la configuración");
    ...
    return (
        <div className='w-full max-w-[660px] font-syne pb-10'>
            <p className='px-2.5 py-0.5 w-fit text-[#7A5AF8] rounded-[50px]  border border-[#EDEEEF] text-[10px] font-medium mb-5 font-syne'>PRESENTIA</p>
            <div className=''>

                <h2 className='mb-4 text-black text-[26px] font-normal font-unbounded '>Elige tus proveedores de contenido</h2>
                <p className='text-[#000000CC] text-xl font-normal font-syne'>Selecciona los motores de IA que generarán el texto y las imágenes de tus diapositivas.</p>
            </div>
            <div className='flex items-center gap-2 bg-[#F0F3F9B2] rounded-[8px]  px-6 py-2.5 my-[54px]'>
                ...
                <p className='text-sm text-[#5F6062] font-medium'>Se ejecuta localmente en tu dispositivo. Tus API keys y configuración de generación permanecen en tu máquina.</p>
            </div>
            ...
            <div className='p-3 border border-[#EDEEEF] rounded-[11px] bg-white '>
                <div className="flex items-center gap-[24.3px]  mb-[42px]">
                    ...
                    <div className='w-full'>
                        ...
                        <h3 className="text-xl font-normal text-[#191919] pb-1.5">Configuración de generación de texto</h3>
                        <p className=" text-sm  text-gray-500">
                            Elige de dónde proviene el contenido de texto
                        </p>
                    </div>
                </div>
                <CodexConfig
                ...
                <div className='flex items-center gap-2.5 my-[30px]'>
                    ...
                    <p className='text-xs font-normal text-[#999999]'>O</p>
                    <div className='w-full h-[1px] bg-[#E1E1E5]' />
                </div>
                <div className="flex w-full max-w-[222px] flex-col items-start gap-4">
                    <div className="flex w-full flex-col justify-start">
                        ...
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                            Seleccionar proveedor de texto
                        </label>
                        <Popover
                            ...
                            {llmConfig.LLM === 'ollama' ? (
                                <>
                                    {!useCustomOllamaUrl ? (
                                        ...
                                        <>
                                            ...
                                            >
                                                Usar URL de Ollama por defecto
                                            </button>
                                        </>
                                    )}
                                </>
                                ...
                                <>
                                    <div className='flex items-center justify-between mb-2'>
                                        ...
                                        {llmConfig.LLM && LLM_PROVIDERS[llmConfig.LLM!]?.getApiKeyUrl && <a href={LLM_PROVIDERS[llmConfig.LLM!]?.getApiKeyUrl || ""} target='_blank' className='text-[#666666] text-xs font-normal flex items-center gap-1'>Obtener API Key <ArrowUpRight className='w-3.5 h-3.5' /></a>}
                                    </div>
            ...
            <div className={`p-3 border border-[#EDEEEF] rounded-[11px] relative mt-5 bg-white ${llmConfig.DISABLE_IMAGE_GENERATION ? "bg-[#F9FAFB]" : ""}`}>
                <ToolTip content="Enable/Desactivar generación de imágenes" className='flex justify-end items-center absolute top-3 right-3'>
                ...
                <div className={` flex items-center gap-6 ${llmConfig.DISABLE_IMAGE_GENERATION ? "" : "mb-[42px]"}`}>
                    ...
                    <div>
                        ...
                        <h3 className="text-xl font-normal text-[#191919] ">Configuración de generación de imágenes</h3>
                        <p className=" text-sm  text-gray-500">
                            Elige de dónde provienen las imágenes
                        </p>
                    </div>
                </div>
                {!llmConfig.DISABLE_IMAGE_GENERATION && (
                    <div className='flex flex-col gap-4'>
                        ...
                        <div className="w-full">
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                Seleccionar proveedor de imágenes
                            </label>
                            <div className="w-full">
                                <Popover
                                ...
                                return (
                                    <div className="w-full ">
                                        <div className='flex items-center justify-between mb-2'>
                                            ...
                                            {provider.getApiKeyUrl && <a href={provider.getApiKeyUrl || ""} target='_blank' className='text-[#666666] text-xs font-normal flex items-center gap-1'>Obtener API Key <ArrowUpRight className='w-3.5 h-3.5' /></a>}
                                        </div>
                                        <div className="relative">
                                            <input
                                                type={showApiKey ? 'text' : 'password'}
                    ...
                    <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-2xl max-w-md w-full p-6 relative">
                        ...
                        <div className="text-center">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">
                                {downloadingModel.done ? "¡Descarga completa!" : "Descargando modelo"}
                            </h3>
                            ...
                            {downloadingModel.status && downloadingModel.status !== "pulled" && (
                                <div className="text-xs text-gray-500">
                                    {downloadingModel.status === "downloading" && "Descargando archivos del modelo..."}
                                    {downloadingModel.status === "verifying" && "Verificando integridad del modelo..."}
                                    {downloadingModel.status === "pulling" && "Descargando modelo del registro..."}
                                </div>
                            )}
