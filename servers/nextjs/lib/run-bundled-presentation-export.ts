...
export function getPresentIAAppRoot(): string {
  return (
    process.env.PRESENTIA_APP_ROOT?.trim() ||
...
function extractSessionTokenFromCookieHeader(cookieHeader?: string): string | undefined {
  ...
  const match = cookieHeader.match(/(?:^|;\s*)presentia_session=([^;]+)/);
  if (!match?.[1]) {
    return undefined;
  }
...
function bundledConverterPath(exportRoot: string): string {
  ...
  if (fromEnv && fromEnv.indexOf("convert-linux-x64") === -1) {
    return fromEnv;
  }
...
async function runBundledPresentationExportLocked(params: {
  ...
  const appRoot = getPresentIAAppRoot();
  ...
  const tempBase =
    process.env.TEMP_DIRECTORY?.trim() || path.join(os.tmpdir(), "presentia");
