...
const DocumentsPreviewPage: React.FC = () => {
  ...
  const maintainDocumentTexts = async () => {
    ...
    if (promises.length > 0) {
      ...
      try {
        ...
        notify.error("No se pudo leer el documento", "Error al leer el contenido del documento.");
      }
  ...
  const handleCreatePresentation = async () => {
    try {
      setShowLoading({
        message: "Generando esquema de la presentación...",
        show: true,
        duration: 40,
        progress: true,
      });
      ...
      notify.error("Error al crear", error.message || "Ocurrió un error al crear la presentación.");
      setShowLoading({
      ...
      <div className="h-full mr-4">
        <div className="overflow-y-auto custom_scrollbar h-full">
          <div className="h-full w-full max-w-full flex flex-col mb-5">
            <h1 className="text-2xl font-medium mb-5">Contenido:</h1>
    ...
    <div className={`bg-white/90 min-h-screen flex flex-col w-full`}>
      ...
      <div className="flex mt-6 gap-4 font-instrument_sans">
        ...
        <div className="fixed bottom-5 right-5">
          ...
          >
            <span className="text-white font-semibold">Siguiente</span>
            <ChevronRight />
          </Button>
        </div>
      </div>
