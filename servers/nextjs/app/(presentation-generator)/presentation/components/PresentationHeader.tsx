...
const PresentationHeader = ({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        notify.error("No se pudieron cargar los temas", e?.message || "Error al cargar los temas.");
      }
    };
    if (themes.length === 0) {
      load();
  ...
  const commitTitleEdit = () => {
    ...
    const next = trimmed || presentationData.title || "Presentación";
  ...
  const handleExportPptx = async () => {
    ...
    try {
      ...
      exportToastId = notify.loading(
        "Exportando PPTX",
        "Tu presentación se está exportando. Esto puede tardar un momento."
      );
      setIsExporting(true);
      ...
      if (window.electron?.exportPresentation) {
        ...
        if (!response.ok) {
          throw new Error("Error al exportar PPTX");
        }
      ...
      notify.success(
        ...
        "Tu archivo PPTX ha sido descargado.",
        { id: exportToastId }
      );
    } catch (error) {
      ...
      notify.error(
        ...
        "Tenemos problemas para exportar tu presentación. Por favor, inténtalo de nuevo.",
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      notify.success(
        "Exportación completada",
        "Tu archivo PDF ha sido descargado.",
        { id: exportToastId }
      );
    } catch (err) {
      console.error(err);
      ...
      {isEditingTitle ? (
          ...
          <div className="flex shrink-0 items-center gap-0.5 border-l border-[#EDECEC] pl-1 ml-0.5">
            ...
            <ToolTip content="Cancelar · Esc">
              <button
                ...
                aria-label="Cancelar edición de título"
              >
        ...
        >
          <h2 className="min-w-0 flex-1 font-unbounded text-lg w-[450px] leading-snug text-[#101323]">
            <MarkdownRenderer
              content={presentationData?.title || "Presentación"}
      ...
      <div className="py-[18px] px-4 sticky top-0 bg-white z-50 shadow-sm font-syne flex justify-between items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          {presentationData && !isStreaming && !isEditingTitle ? (
            <ToolTip content="Renombrar presentación">{titleBlock}</ToolTip>
          ) : (
            titleBlock
          )}
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-2 bg-[#F6F6F9] px-3.5 h-[38px] border border-[#EDECEC] rounded-[80px]">
            <ToolTip content="Regenerar presentación">
              <button
                type="button"
        ...
        <DialogContent className="w-[360px] rounded-2xl border-0 p-0 shadow-2xl sm:max-w-[360px]">
          <DialogHeader className="items-center px-6 pb-4 pt-6 text-center">
            ...
            <DialogDescription className="text-sm leading-relaxed text-gray-500">
              Esto reemplazará las diapositivas actuales con una versión recién generada y borrará el historial de deshacer. Es posible que se pierdan tus ediciones actuales.
            </DialogDescription>
          </DialogHeader>
