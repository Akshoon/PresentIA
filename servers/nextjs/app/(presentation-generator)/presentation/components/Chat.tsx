...
const suggestions: { id: string; icon: ReactNode; suggestion: string }[] = [
  {
    ...
    suggestion: "Generar una presentación completa a partir de mi tema",
  },
  {
    id: "improve",
    icon: (
    ...
    suggestion: "Mejorar el contenido de esta diapositiva",
  },
  {
    id: "rewrite",
    icon: (
    ...
    suggestion: "Reescribir este contenido de forma profesional",
  },
  {
    id: "notes",
    icon: (
    ...
    suggestion: "Añadir notas del orador a esta diapositiva",
  },
];

const quickPrompts = [
  "Expandir cada sección",
  "Reordenar para contar una historia",
  "Añadir secciones faltantes",
  "Convertir en flujo de pitch",
];
...
const conversationStorageKey = (presentationId: string) =>
  `presentia:chat:conversationId:${presentationId}`;
...
const TOOL_LABELS: Record<string, string> = {
  getPresentationOutline: "Lector de esquema",
  searchSlides: "Buscador de diapositivas",
  getSlideAtIndex: "Lector de diapositiva",
  getPresentationThemeCatalog: "Catálogo de temas",
  getAvailableLayouts: "Buscador de diseños",
  getContentSchemaFromLayoutId: "Validador de esquema",
  generateAssets: "Generador de recursos",
  saveSlide: "Guardador de diapositiva",
  deleteSlide: "Eliminador de diapositiva",
...
const humanizeTraceMessage = (message: string, tool?: string) => {
  ...
  if (lower === "reading deck context") {
    return "Revisando el contexto de tu presentación.";
  }
  if (lower === "reading the presentation outline") {
    return "Leyendo el esquema de la presentación.";
  }
  if (lower === "searching relevant slides") {
    return "Buscando diapositivas con contenido relevante.";
  }
  if (lower === "opening the requested slide") {
    return "Abriendo la diapositiva seleccionada.";
  }
  if (lower === "checking available themes") {
    return "Comprobando los temas de color disponibles.";
  }
  if (lower === "checking available layouts") {
    return "Comprobando los diseños disponibles.";
  }
  ...
  if (lower.endsWith("completed.")) {
    return trimmed.replace(/completed\./i, "completado.");
  }
  if (lower.includes("failed")) {
    return trimmed.replace(/failed/i, "falló");
  }
  return trimmed;
};
...
const formatTraceActivity = (
  ...
  if (trace.tool && trace.status === "start") {
    return {
      label: `Ejecutando ${getToolLabel(trace.tool)}...`,
      kind: trace.kind,
      round: trace.round,
      tool: trace.tool,
      state: "running",
  ...
  if (trace.tool && trace.status === "success") {
    return {
      label: `${getToolLabel(trace.tool)} completado.`,
      kind: trace.kind,
      round: trace.round,
      tool: trace.tool,
      state: "success",
  ...
  if (trace.tool && trace.status === "error") {
    return {
      label: `${getToolLabel(trace.tool)} falló.`,
      kind: trace.kind,
      round: trace.round,
      tool: trace.tool,
      state: "error",
  ...
  if (
    ...
    return {
      label: `Planificando herramientas: ${trace.tools
...
const Chat = ({
    ...
    const run = async () => {
      try {
        ...
        const detail =
          error instanceof Error
            ? error.message
            : "No se pudo cargar el chat anterior";
        notify.error("No se pudo cargar el chat", detail);
      } finally {
        if (!cancelled) {
  ...
  const refreshPresentationIncrementally = useCallback(async () => {
    ...
    try {
      ...
      notify.error("Fallo al actualizar", "Se guardaron las diapositivas, pero falló la actualización.");
    } finally {
  ...
  const refreshPresentationIfNeeded = async (toolCalls: string[]) => {
    ...
    try {
      ...
      notify.error(
        "Fallo al actualizar",
        "El chat se completó, pero falló la actualización de las diapositivas."
      );
    }
  };
  ...
  const submitMessage = async (rawMessage: string) => {
    ...
    if (!presentationId) {
      notify.error(
        "Presentación no lista",
        "La presentación aún no está lista."
      );
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Fallo al enviar el mensaje de chat";
      ...
      notify.error("Error de chat", message);
    } finally {
      setActiveMutationToolCount(0);
    ...
    <div className="flex h-full w-full flex-col bg-white">
                          ...
                          {message.toolCalls &&
                            message.toolCalls.length > 0 && (
                              <div className="pt-0.5 text-[11px] text-[#98A2B3]">
                                Herramientas llamadas: {message.toolCalls.join(", ")}
                              </div>
                            )}
                        </div>
                      )}
      ...
      >
        <textarea
          ...
          placeholder="Mejora tus diapositivas..."
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2 border border-[#EDEEEF] bg-white px-3 py-1 rounded-[64px]">
            <button
              ...
              title="Aún no se admiten archivos adjuntos"
            >
            ...
            <ToolTip
              content={
                isFollowAgentEnabled
                  ? "Desactivar modo seguir a la IA"
                  : "Activar modo seguir a la IA"
              }
            >
              <button
                type="button"
                ...
                title={
                  isFollowAgentEnabled
                    ? "Seguir a la IA activado: salto automático a la diapositiva activa"
                    : "Seguir a la IA desactivado"
                }
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="ml-auto flex items-center gap-2">
            {isSending ? (
              <button
                ...
                aria-label="Detener respuesta del chat"
              >
                <Loader2
