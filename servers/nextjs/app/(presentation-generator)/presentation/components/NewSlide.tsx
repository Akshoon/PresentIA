...
const LayoutItem = memo(({ layout, onSelect }: LayoutItemProps) => {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      aria-label={`Añadir diseño de ${layoutName || "diapositiva"}`}
      title={layoutName || "Diseño de diapositiva"}
...
const NewSlideV1 = ({
  ...
  const handleNewSlide = useCallback(
    (sampleData: any, id: string) => {
      try {
        ...
        console.error(error);
        notify.error("No se pudo añadir la diapositiva", "Ocurrió un error al añadir la nueva diapositiva.");
      }
    },
    [
      index,
  ...
  const layoutCountText = `${layouts.length} diseño${
    layouts.length === 1 ? "" : "s"
  } disponible${layouts.length === 1 ? "" : "s"}`;
    ...
    >
      ...
      <div className="flex min-h-[64px] items-center justify-between border-b border-[#EDEEEF] px-5 py-4 md:px-6">
        <div>
          ...
          >
            Elegir diseño de diapositiva
          </h2>
          <p className="mt-1 text-xs font-normal leading-none text-[#7A7A85]">
            {loading ? "Cargando diseños..." : layoutCountText}
          </p>
        </div>
        {loading && (
