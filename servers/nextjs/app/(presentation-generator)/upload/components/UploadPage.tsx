...
const UploadPage = () => {
  ...
  const ensureStockImageProviderReady = async (): Promise<boolean> => {
    ...
    try {
      ...
      notify.error(
        "Proveedor de imágenes no disponible",
        error?.message ||
        `No se puede acceder a ${selectedProvider} en este momento. Por favor, verifica tu clave de API/configuración e inténtalo de nuevo.`
      );
      return false;
    }
  };
  ...
  const validateConfiguration = (): boolean => {
    if (!config.language) {
      ...
      notify.warning("Idioma requerido", "Por favor, selecciona un idioma.");
      return false;
    }

    if (files.length > 0 && config.language === LanguageType.Auto) {
      trackUploadValidationFailure("language_auto_with_documents");
      notify.warning("Idioma requerido", "Por favor, elige un idioma antes de procesar los documentos subidos.");
      return false;
    }

    if (!config.prompt.trim() && files.length === 0) {
      trackUploadValidationFailure("prompt_or_document_missing");
      notify.warning("Entrada requerida", "Proporciona un prompt o sube al menos un documento.");
      return false;
    }
    return true;
  };
  ...
  const handleDocumentProcessing = async () => {
    setLoadingState({
      ...
      showProgress: true,
      duration: 90,
      extra_info: files.length > 0 ? "Puede tomar unos minutos para documentos grandes." : "",
    });
  ...
  const handleGenerationError = (error: any) => {
    ...
    notify.error(
      "Error de generación",
      error.message || "Ocurrió un error al iniciar tu presentación."
    );
  };
  ...
  return (
    <Wrapper className="pb-10 lg:max-w-[65%] xl:max-w-[60%]">
      ...
      <div className="rounded-2xl " >
        ...
        <div className="p-4 ">
          <h3 className="text-sm font-medium text-[#333333] mb-2">Archivos adjuntos (opcional)</h3>
          <SupportingDoc
            files={[...files]}
