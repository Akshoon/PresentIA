...
const SupportingDoc = ({
    ...
    const handleValidate = (filesToReview: File[]) => {
        ...
        if (disallowed.length > 0) {
            notify.error('Algunos archivos no están soportados', 'Soportado: Word, PowerPoint, hojas de cálculo, PDF/TXT y archivos de imagen.')
        }
    }
    ...
    const applyFileLimit = (candidateFiles: File[]) => {
        ...
        notify.warning('Límite máximo de archivos alcanzado', `Solo puedes subir hasta ${MAX_SUPPORTED_FILES} documentos.`)
    ...
    const handleFilesSelected = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (allowedFiles.length > files.length) {
            notify.success('Archivos seleccionados', `Se han añadido ${allowedFiles.length - files.length} archivo(s).`)
        }
        ...
        <div className="space-y-2" data-testid="attachments-uploader">
            <div className="flex items-center justify-between">
                <p className="text-sm text-gray-600 font-syne">
                    {hasFiles ? `${filteredFiles.length} archivo${filteredFiles.length > 1 ? 's' : ''} adjunto${filteredFiles.length > 1 ? 's' : ''}` : ''}
                </p>
                {hasFiles && <button
                    type="button"
            ...
            >
                ...
                <div className="flex flex-col items-center gap-2">
                    ...
                    <p className='text-[#808080] text-sm  font-normal'>(Docs de Office, hojas de cálculo, imágenes, PDF/TXT)</p>
                </div>
            </label>
                ...
                <div className="mt-2">
                    ...
                    {filteredFiles.length !== files.length && (
                        <p className="mt-2 text-xs text-amber-600 font-syne">
                            Se omitieron algunos archivos. Soportado: Word, PowerPoint, hojas de cálculo, PDF/TXT y archivos de imagen.
                        </p>
                    )}
                </div>
            )}
