...
const SettingsPage = () => {
  ...
  const [mode, setMode] = useState<'nanobanana' | 'presentia'>('presentia')
  ...
  const ensureSelectedStockProviderReady = async (): Promise<boolean> => {
    ...
    try {
      ...
      notify.error(
        ...
        error?.message ||
        `No se puede conectar con ${provider} usando la clave API proporcionada. Por favor, verifica tu configuración e inténtalo de nuevo.`
      );
      return false;
    }
  };
  ...
  const handleSaveConfig = async () => {
    ...
    if (validationError) {
      notify.warning("No se puede guardar la configuración", validationError);
      if (
    ...
    try {
      ...
      notify.success(
        ollamaModelDownloaded ? "Configuración guardada y modelo listo" : "Configuración guardada",
        ollamaModelDownloaded
          ? "Tu configuración se guardó y el modelo de Ollama terminó de descargarse."
      ...
      notify.error("No se pudo guardar la configuración", message);
      setButtonState((prev) => ({
        ...prev,
        isLoading: false,
        isDisabled: false,
  ...
  const handleModelDownload = async (): Promise<"completed" | "cancelled"> => {
    ...
    try {
      ...
      if (aborted) {
        ...
        notify.info(
          "Descarga cancelada",
          "La descarga del modelo Ollama fue cancelada. Tu configuración ya está guardada. Puedes guardar de nuevo para reintentar la descarga."
        );
        return "cancelled";
      }
      setDownloadingModel(null);
  ...
  useEffect(() => {
    if (
      ...
      setButtonState({
        isLoading: true,
        isDisabled: true,
        text: `Descargando modelo (${percentage}%)`,
        showProgress: true,
  ...
  useEffect(() => {
    ...
    if (
      ...
      notify.error("No se puede guardar la configuración", "Por favor, selecciona un modelo para el proveedor seleccionado");
      ...
      const handleClick = (e: MouseEvent) => {
        ...
        if (
          ...
          // notify.error("No se puede guardar la configuración", "Please select a model for the selected provider");
          e.preventDefault();
    ...
    <div className="h-screen font-syne flex flex-col overflow-hidden relative">
      <main className="w-full mx-auto gap-6   overflow-hidden flex ">
        ...
        <div className="w-full">
          ...
          {mode === 'presentia' && selectedProvider === 'text-provider' && <TextProvider
            ...
            llmConfig={llmConfig}
          />}
          {mode === 'presentia' && selectedProvider === 'image-provider' && <ImageProvider llmConfig={llmConfig} setLlmConfig={setLlmConfig} />}
          {mode === 'presentia' && selectedProvider === 'web-search-provider' && <WebSearchProvider llmConfig={llmConfig} setLlmConfig={setLlmConfig} />}
          ...
          {selectedProvider === "session" && (
            <div className="w-full max-w-lg space-y-5 rounded-[20px] border border-[#EDEEEF] bg-white p-7">
              <div>
                <h4 className="font-unbounded text-lg font-normal text-black">Cerrar sesión</h4>
                <p className="mt-2 font-syne text-sm leading-relaxed text-[#494A4D]">
                  Cierra tu sesión en esta instancia. Tendrás que iniciar sesión de nuevo para usar la app y acceder al API.
                </p>
              </div>
              <LogoutButton
                label="Cerrar sesión"
                ...
                downloadingModel.status !== "pulled" && (
                  <div className="text-xs text-gray-500">
                    ...
                    {downloadingModel.status === "pulling" &&
                      "Descargando modelo del registro..."}
                  </div>
                )}
