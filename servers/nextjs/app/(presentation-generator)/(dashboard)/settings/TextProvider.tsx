    ...
    try {
      ...
      if (response.ok) {
        ...
        notify.error(
          ...
          `El servidor no pudo listar los modelos de ${modelLabel}. Verifica tu clave API o endpoint e inténtalo de nuevo.`
        );
      }
    } catch (error) {
      ...
      notify.error(
        "No se pudieron cargar los modelos",
        "Algo salió mal al contactar al proveedor. Verifica tu red e inténtalo de nuevo."
      );
      setAvailableModels([]);
      setModelsChecked(true);
    } finally {
              ...
              <div className="flex flex-col justify-start w-full ">
                  ...
                  <>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder={
                          selectedProvider === "litellm"
                            ? "Opcional si tu proxy no requiere autenticación"
                            : `Introduce tu ${providerApiKeyLabel}`
                        }
                      />
                      <button
                        type="button"
                ...
                {selectedProvider === "custom" && (
                  <input
                    ...
                    placeholder="URL compatible con OpenAI"
                  />
                )}
                {selectedProvider === "litellm" && (
                  <>
                    ...
                    <input
                      ...
                      className="w-full px-2 py-3 outline-none border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                      placeholder="ej. http://host.docker.internal:4000/v1"
                    />
                    <p className="mt-1.5 text-xs text-gray-500">
                      Raíz compatible con OpenAI (normalmente termina en /v1); se añade /v1
                      si se omite. La clave API anterior es opcional para
                      proxies locales sin autenticación.
                    </p>
                  </>
                )}
                {selectedProvider === "lmstudio" && (
                  <>
                    ...
                    />
                    <p className="mt-1.5 text-xs text-gray-500">
                      Por defecto es localhost:1234/v1, y se añade /v1
                      automáticamente cuando se omite.
                    </p>
                  </>
                )}
                {selectedProvider === "fireworks" && (
                  <>
                    <label className="mt-3 block text-sm font-medium text-gray-700 mb-2">
                      URL base de Fireworks (opcional)
                    </label>
                    <input
                      type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-3">
                  {selectedProvider === "ollama"
                    ...
                    : `Seleccionar modelo de ${modelLabel}`}
                </label>
                    ...
                    <PopoverTrigger asChild>
                      ...
                      >
                        <span className="text-sm truncate font-medium text-gray-900">
                          {(() => {
                            if (!currentModel) return "Selecciona un modelo";
      ...
      {modelsChecked && availableModels.length === 0 && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            No se encontraron modelos. Por favor, asegúrate de que las credenciales de tu proveedor son
            válidas y el proveedor seleccionado es accesible.
          </p>
        </div>
      )}
