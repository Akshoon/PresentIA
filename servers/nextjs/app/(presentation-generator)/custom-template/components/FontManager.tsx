      ...
      <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-[#F3F4F6] bg-[#FAFAFA]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-semibold text-[#111827]">Gestión de Fuentes</h2>
              <p className="text-sm text-[#6B7280] mt-0.5">
                {allFontsUploaded
                  ? "¡Todas las fuentes están listas! Puedes continuar con la vista previa."
                  : "Sube las fuentes que faltan para asegurar que tu presentación se muestre correctamente."}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="p-4 bg-[#F0FDF4] rounded-xl border border-[#BBF7D0]">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-sm font-semibold text-[#166534]">
                  Fuentes Disponibles ({fontsData.available_fonts.length})
                </h4>
              </div>
            ...
            <div className="p-4 bg-[#FFFBEB] rounded-xl border border-[#FDE68A]">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-sm font-semibold text-[#92400E]">
                  Fuentes Faltantes ({fontsNeedingUpload.length})
                </h4>
              </div>
                      ...
                      <div>
                        ...
                        {font.family_name && font.family_name !== font.name && (
                          <span className="text-xs text-[#6B7280] block">
                            Familia: {font.family_name}
                            {font.variant ? ` · ${font.variant.replace(/_/g, " ")}` : ""}
                          </span>
                        )}
                        <span className="text-xs text-[#9CA3AF]">
                          El archivo a subir debe coincidir exactamente con este nombre (.ttf, .otf, .woff, .woff2, .eot)
                        </span>
                      </div>
                    </div>
                    <div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            {!allFontsUploaded && (
              <div className="flex items-start gap-2 text-sm text-[#6B7280]">
                ...
                <p>Puedes continuar sin todas las fuentes, pero es posible que algunos textos no se muestren correctamente.</p>
              </div>
            )}
            {allFontsUploaded && (
              <p className="text-sm text-[#16A34A] font-medium">
                ✓ Todas las fuentes están listas
              </p>
            )}
            <Button
              size="lg"
            ...
            >
              {isUploading ? (
                ...
                <>
                  {allFontsUploaded ? 'Continuar a la Vista Previa' : 'Continuar'}
