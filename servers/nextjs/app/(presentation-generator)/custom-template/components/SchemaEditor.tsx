...
export const SchemaEditor: React.FC<SchemaEditorProps> = ({
    ...
    const handleFillContent = async (mode: 'min' | 'normal' | 'max') => {
        ...
        try {
            ...
            notify.success("Contenido generado", `El contenido ${modeLabels[mode]} se generó correctamente.`);
            handleCancel();
        } catch (error) {
            console.error('Error generating content:', error);
            notify.error("Error de generación", error instanceof Error ? error.message : "No se pudo generar el contenido.");
        } finally {
                ...
                <div className="px-4 py-3 h-[calc(100vh-350px)] overflow-y-auto custom_scrollbar pb-10">
                    ...
                    <div className="mb-3">
                        ...
                        <p className="text-sm text-gray-600 py-1">
                            Establece límites de caracteres mín/máx para cada campo. Esto controla cuánto texto genera la IA para tu diapositiva.
                        </p>
                    </div>
                    ...
                    {parseError ? (
                        ...
                        <div className="text-center py-8 text-gray-400">
                            ...
                            <p className="text-xs">No hay campos editables</p>
                        </div>
                    ) : (
                        <div className="space-y-1 px-2 ">
                            {hierarchicalFields.map(({ field, children }) => {
                                            ...
                                            {fExpanded && (
                                                <div className="px-2.5 pb-2.5 pt-1.5 border-t border-gray-50">
                                                    ...
                                                    {(f.originalType === 'string' || f.originalType === 'number') && (
                                                        <div className="mb-2">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                {isTypeChanged(f) && (
                                                                    <span className="text-[9px] px-1 py-0.5 bg-blue-50 text-blue-500 rounded">cambiado</span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    )}
                                                    ...
                                                    {f.type === 'array' && (
                                                        <div className="grid grid-cols-2 gap-2">
                                                            <div>
                                                                <Label className="text-xs text-gray-600 mb-1 block">Mín. elementos</Label>
                                                                <Input
                                                                    type="number"
                                                                    min={0}
                                                                    value={f.minItems ?? ''}
                                                            ...
                                                            <div>
                                                                <Label className="text-xs text-gray-600 mb-1 block">Máx. elementos</Label>
                                                                <Input
                                                                    type="number"
                                                                    min={0}
                                                                    value={f.maxItems ?? ''}
                                                    ...
                                                    {f.default !== undefined && typeof f.default !== 'object' && (
                                                        <div className="mt-3">
                                                            <Label className="text-xs text-gray-600 mb-1 block">Valor actual</Label>
                                ...
                                const headerLabel = isArrayParent ? 'Campos del Elemento' : `Propiedades de ${formatFieldName(field.name)}`;
